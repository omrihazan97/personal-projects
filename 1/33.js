...
// __    this.work_hours = work_hours;_____זה המערך שאמרתה לי להגדיר לכל עובד אבל אני לא מצליח  להוסיך לו ערכים
...
class worker {
  ...
  hours() {
    document.getElementById("ok").addEventListener("click", () => {
      ...
      // console.log(w1.getMessage());
      console.log(this.work_hours);
      this.work_hours.push(houres);
      localStorage.setItem("worker", JSON.stringify(this.work_hours));
    });
  }

  // ____זה המשתנה שמחזיק בתוכו את התוצאה הסופית של שעות העהבודה _>>>>_  let hisov = gg_exit - gg_Entrance;____________<______________פה יש חלק שהוא בעייתי לי אשמח לעזרה______________________________________________
  //זה דוחף אותו למערך הקיים ולא למערך בקונסטרקטור
  // לכל עובד שנוצר הוא נמוצר עם מערך פרטי משלו אני לא מצליח לגשת אותו מערך פרטי משלו
  //בנוסף צריך ליצור את האפשררות להשתמש בחיפוש עובד הקיים בבלוק להגדיר עובד ורק לו ספיציפת להוסיף שעות עבודה כל פעם
  //לאחר מכן לייצר הדפסה של הנתונים לדף הוואב לבלוק הקיים
  fun_inout() {
